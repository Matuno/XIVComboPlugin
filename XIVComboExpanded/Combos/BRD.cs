...
namespace XIVComboExpandedestPlugin.Combos
{
    ...
    internal class BardPerfectSidesFeature : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            ...
            {
                var gauge = GetJobGauge<BRDGauge>();
                if (gauge.Song == Song.WanderersMinuet && (gauge.Repertoire == 3 || (!IsActionOffCooldown(BRD.Sidewinder) && gauge.Repertoire > 0 && !IsEnabled(CustomComboPreset.BardPerfectSidesOnlyFullOption))))
    ...
    internal class BardRadiantStrikesFeature : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            if (level < BRD.Levels.RadiantFinale || (IsActionOffCooldown(BRD.RagingStrikes) && !IsActionOffCooldown(BRD.RadiantFinale)))
                return BRD.RagingStrikes;
            return actionID;
            /* return actionID == BRD.RadiantFinale && ((IsActionOffCooldown(BRD.RagingStrikes) && !IsActionOffCooldown(BRD.RadiantFinale)) || (!CanUseAction(OriginalHook(BRD.RadiantFinale))) ||
